...
| `game/__init__.py` | `TetrisGame`・`Renderer` をパッケージ外へエクスポート |
...
step2. grid = [] を用意し、二重ループで COLS 個の 0 を持つ行を ROWS 個作って二次元リストにする
...
step3. ランダムに新しいミノを生成して piece に代入する
         random.choice(list(TETRIMINO_SHAPES.values())) でデータを選ぶ
...
step3. 取り除いた行数だけ、COLS 個の 0 を持つ空行を先頭に追加する
         → ブロックが上から落ちてくる見た目になる
...
def clear_lines(grid, total_lines):
    ...
    total_lines += cleared
    return empty_rows + new_grid, total_lines
```
